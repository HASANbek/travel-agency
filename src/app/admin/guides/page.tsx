...
export default function GuidesPage() {
  ...
  async function load() {
    setLoading(true);
    const [citiesRes, guidesRes] = await Promise.all([
      fetch("/api/admin/cities"),
      fetch(filterCityId ? `/api/admin/guides?cityId=${filterCityId}` : "/api/admin/guides"),
    ]);
  ...
  async function handleDelete(id: number) {
    if (!confirm(t.guides.confirmDelete)) return;
  ...
  return (
    <div>
      <PageHeader title={t.guides.title} description={t.guides.description} />
      ...
      <Card className="p-5 mb-6">
        <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3">
          ...
          <Field label={t.guides.nameLabel} className="w-52">
            <Input
              value={form.name}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-white/10 text-left text-xs uppercase tracking-wide text-gray-400">
                ...
                <th className="px-5 py-3 font-medium">{t.guides.colName}</th>
                  ...
                  {editingId === g.id ? (
                    <>
                      ...
                      <td className="px-5 py-2.5 text-right space-x-1">
                        <IconButton onClick={() => handleUpdate(g.id)}>{t.common.save}</IconButton>
                        <IconButton onClick={() => setEditingId(null)}>{t.common.cancel}</IconButton>
                      </td>
                    </>
                  ) : (
                    <>
                      <td className="px-5 py-3 font-medium">{g.city.name}</td>
                      <td className="px-5 py-3">{g.name}</td>
                      <td className="px-5 py-3 text-gray-500 dark:text-white/50">
                        {localizedDescription(g, lang) || "—"}
                      </td>
                      <td className="px-5 py-3 text-gray-500 dark:text-white/50">
                        {g.phone || "—"}
                      </td>
                      <td className="px-5 py-3 tabular-nums">{formatUsd(g.priceUsd)}</td>
                      <td className="px-5 py-3 tabular-nums">{formatUzs(g.priceUzs)}</td>
                      <td className="px-5 py-3 text-right space-x-1">
                        <IconButton onClick={() => startEdit(g)}>{t.common.edit}</IconButton>
                        <IconButton variant="danger" onClick={() => handleDelete(g.id)}>
                          {t.common.delete}
                        </IconButton>
                      </td>
                    </>
